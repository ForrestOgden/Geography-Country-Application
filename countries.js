...
const imgContainerHeading = document.querySelector(".img-container-heading");
    ...
    request.addEventListener("load", function () {
      ...
      nav.addEventListener("click", function (e) {
        if (e.target.classList.contains("country-name")) {
          ...
          // before fetch, clear innerHTML and heading of imgContainer
          imagesBox.style.display = "flex";
          imgContainer.style.display = "flex";
          imgContainer.innerHTML = "";

          imagesBox.style.backgroundColor = Math.floor(
            Math.random() * 16777215
          ).toString(16);

          // now set the heading text Content:

          const headingHTML = `<h2 class='img-container-heading'>Pictures of ${countryName}:</h2>`;
          imgContainer.insertAdjacentHTML("beforeend", headingHTML);

          const apiKey = `28519618-7bdc3346497809f85f8537464`;

          fetch(
            `https://pixabay.com/api/?key=${apiKey}&q=${countryName}&image_type=photo`
          )
            .then((res) => res.json())
            .then((data) => {
              console.log(data);
              ...
              if (data.hits.length < 1) {
                imgContainer.insertAdjacentHTML(
                  "beforeend",
                  `<p class='error-msg'>No images were found for this country/territory! Try another one.</p>`
                );
              }

              const imgURLS = imgs.map((img) => img.largeImageURL);

              imgURLS.forEach((url, i) => {
                if (i < 15) {
                  const imgHTML = `<img class='image' src='${imgURLS[i]}' alt='' >`;
